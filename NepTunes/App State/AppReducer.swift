...
let appReducer = AppReducer.combine(
    ...
    playerScrobblerReducer, // it's important that this reducer runs before the playerReducer, so the currentTrack isn't set yet
    playerReducer
        .pullback(
            state: \.playerState,
...
let playerScrobblerReducer = Reducer<AppState, AppAction, SystemEnvironment<AppEnvironment>> { state, action, environment in
    switch action {
