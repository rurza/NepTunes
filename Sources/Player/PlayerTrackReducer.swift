...
public let playerTrackReducer = Reducer<PlayerState, PlayerTrackAction, SystemEnvironment<PlayerEnvironment>> { state, action, environment in
    ...
    case let .newTrack(track):
        ...
        if track.duration == nil {
            let player = environment.localEnvironment.playerForPlayerType(playerType)
    ...
    case let .trackDoesNotHaveBothArtworkAndArtworkURL(track):
        guard let playerType = state.currentPlayerState?.playerType else { return .none }
        return environment.getTrackInfo(environment.localEnvironment.playerForPlayerType(playerType))
